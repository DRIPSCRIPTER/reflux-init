task.spawn(function() 
	repeat task.wait() until pcall(Instance.new, "Player") 
	printidentity()
end)

task.spawn(function() -- separate cus this is for script triggering
	local RobloxGui = game:GetService("CoreGui"):WaitForChild("RobloxGui")
	local Modules = RobloxGui.Modules

	local BackpackFrame = RobloxGui:FindFirstChild("Backpack")
	...
	local userStore = require(Modules.InGameMenu.Utility.UserLocalStore).new()
	local displayCount = userStore:GetItem(LOCAL_STORAGE_KEY_NATIVE_CLOSE)
	userStore:SetItem(LOCAL_STORAGE_KEY_NATIVE_CLOSE, "1")
	userStore:Flush()
end)
